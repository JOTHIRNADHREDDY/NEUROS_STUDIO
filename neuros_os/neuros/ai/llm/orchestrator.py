...
-----------------------
  ...
  LLMProvider.NVIDIA     DeepSeek V4 Pro via NVIDIA NIM OpenAI-compatible API
...
class LLMOrchestrator:
    ...
    real LLM (Anthropic, NVIDIA, OpenAI, Ollama) with fallback to stub rules.
    ...
    _DEFAULT_MODELS = {
        ...
        LLMProvider.NVIDIA:    "deepseek-ai/deepseek-v4-pro",
    ...
    def __init__(
        ...
        *,
        ...
        # Rate limiting — minimum interval between LLM API calls (seconds)
        self._min_call_interval: float = 0.5  # 500ms debounce
    ...
    def parse(self, text: str) -> Intent:
        ...
        if self._provider == LLMProvider.STUB:
            ...
            # Rate limiting — prevent runaway API cost
            elapsed_since_last = t0 - self._last_call_time
            if elapsed_since_last < self._min_call_interval:
                wait = self._min_call_interval - elapsed_since_last
                logger.debug("[LLM] Rate limit: waiting %.0fms", wait * 1000)
                time.sleep(wait)
    ...
    def _llm_parse(self, text: str) -> Intent:
        ...
        if self._provider == LLMProvider.ANTHROPIC:
            ...
            raw = self._call_nvidia(system, messages)
    ...
    def _call_nvidia(self, system: str, messages: list) -> str:
    ...
    def _resolve_provider(
        ...
        if p == "nvidia":    return LLMProvider.NVIDIA
    ...
    def _env_api_key(self) -> Optional[str]:
        ...
        if self._provider == LLMProvider.NVIDIA:
