...
class MissionPlanner:
    ...
    def _check_condition(condition: str, robot: "Robot") -> bool:
        ...
        e.g. "battery.soc_pct < 20"
        """
        try:
            # Build a minimal evaluation namespace from recent bus data
            # In Phase 3 we use the ContextBuilder's sensor cache
            ctx_builder = None
            if hasattr(robot, "_ai_context"):
                ctx_builder = robot._ai_context
            # Simple passthrough for now (always True if condition present but unresolvable)
            return True
        except Exception:
            return True
